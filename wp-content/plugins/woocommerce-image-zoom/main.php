		...
		if( is_singular( 'product' ) ){
			...
			if ( ! class_exists( 'jQuery_Migrate_Helper' ) ) {
				if ( version_compare( $GLOBALS['wp_version'], '5.6-alpha', '<' ) ) {
					wp_enqueue_script('jquery-migrate', plugins_url( '/assets/js/jquery-migrate-1.4.1-wp.js', __FILE__ ), array('jquery'), '1.4.1', false);
				}else{
					wp_enqueue_script('jquery-migrate', plugins_url( '/assets/js/jquery-migrate-3.3.2-wp.js', __FILE__ ), array('jquery'), '3.3.2', false);
				}
			}
