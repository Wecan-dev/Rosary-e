...
class CompleteOrderAction extends Action {
     ...
     */
    setup(): void {
        Main.getCheckoutForm().off( 'form:validate' );
    ...
    public response( resp: any ): void {
        try {
            if ( resp.result === 'success' && Main.getCheckoutForm().triggerHandler( 'checkout_place_order_success' ) !== false ) {
                // Fire events that need to run before redirect
                jQuery( document.body ).trigger( 'cfw-order-complete-before-redirect', [ Main.getCheckoutForm(), resp ] );
            ...
            if ( resp.messages !== '' ) {
                ...
                jQuery.each( messages.find( '.woocommerce-error li' ), ( i, el ) => {
                ...
                jQuery.each( messages.find( '.woocommerce-info li' ), ( i, el ) => {
                    const alert: Alert = new Alert( Main.getAlertContainer(), <AlertInfo> {
                        type: 'notice',
                        message: jQuery.trim( jQuery( el ).text() ),
                ...
                jQuery.each( messages.find( '.woocommerce-message li' ), ( i, el ) => {
                    const alert: Alert = new Alert( Main.getAlertContainer(), <AlertInfo> {
                        type: 'success',
                        message: jQuery.trim( jQuery( el ).text() ),
    ...
    submitError() {
        ...
        Main.getCheckoutForm().removeClass( 'processing' ).unblock();
