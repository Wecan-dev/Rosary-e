...
class StatCollection extends Singleton {
	...
	public function __construct( $settings_manager ) {
		...
		add_filter( 'cfw_disable_tracking_checkin', array( $this, 'filter_dev_and_staging_domains'), 10, 2 );
	...
	public function send_checkin( $override = false, $ignore_last_checkin = false ) {
		...
		if ( $home_url === $this->cfw_home_site_url || apply_filters( 'cfw_disable_tracking_checkin', false, $home_url ) ) {
			return false;
		}
		...
		$this->setup_data();
		$remote_url = CFW_DEV_MODE && ! isset( $_POST['force-checkin'] ) ? $this->dev_stat_collection_url : $this->stat_collection_url;
		...
		wp_remote_request(
			...
			array(
				...
				'sslverify'   => CFW_DEV_MODE ? false : true,
			)
		);
	 ...
	 */
	private function get_server_database_version() {
		global $wpdb;
	...
	public function setup_data() {
		$data                = array();
		$cfw_version         = defined( 'CFW_VERSION' ) ? CFW_VERSION : Main::instance()->get_version();
	...
	public function updated_option( $key, $old_value, $value ) {
		if ( $key == '_cfw__settings' ) {
			if ( is_array( $value ) &&
                 isset( $value[ $this->allow_tracking_key ] ) &&
                 $value[ $this->allow_tracking_key ] !== null &&
                 isset( $old_value[ $this->allow_tracking_key ] ) &&
                 $old_value[ $this->allow_tracking_key ] !== $value[ $this->allow_tracking_key ] )
			{
	 ...
	 * @return bool
	 */
	function filter_dev_and_staging_domains( $track, $home_url ) {
