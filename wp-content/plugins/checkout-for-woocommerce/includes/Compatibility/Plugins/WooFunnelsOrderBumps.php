...
class WooFunnelsOrderBumps extends Base {
	...
	function add_cfw_positions( $positions ) {
	...
	function position_not_found( $position_id, $position ) {
		add_action( $position['hook'], array( $this, $position_id ), $position['priority'] );
	}

	function position_fragment_not_found( $position_id, $position ) {
		add_action( 'woocommerce_update_order_review_fragments', array( $this, $position_id . "_frg" ), $position['priority'] );
	}

	function cfw_woocommerce_checkout_order_review_above_payment_gateway() {
		$this->print_placeholder( 'cfw_woocommerce_checkout_order_review_above_payment_gateway' );
	}

	function cfw_woocommerce_checkout_order_review_above_payment_gateway_frg( $fragments ) {
	...
	function cfw_woocommerce_checkout_order_review_below_payment_gateway() {
		$this->print_placeholder( 'cfw_woocommerce_checkout_order_review_below_payment_gateway' );
	}

	function cfw_woocommerce_checkout_order_review_below_payment_gateway_frg( $fragments ) {
		$WFOB_Public = \WFOB_Public::get_instance();
	...
	function print_placeholder( $slug ) {
		$WFOB_Public = \WFOB_Public::get_instance();
	...
	function print_position_bump( $position ) {
		if ( empty( $position ) ) {
			return '';
		}
