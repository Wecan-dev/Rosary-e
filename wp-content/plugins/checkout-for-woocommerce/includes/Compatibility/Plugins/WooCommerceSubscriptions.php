...
class WooCommerceSubscriptions extends Base {
	...
	function maybe_hide_shipping_tab( $show_shipping_tab ) {
		...
		foreach( $cart_contents as $i => $cart_item ) {
			...
			} elseif( \WC_Subscriptions_Product::needs_one_time_shipping( $cart_item['data'] ) ) {
				unset( $cart_contents[ $i ] );
			}
		}
	...
	function disable_cfw_for_change_payment_request( $result ) {
