...
class Main extends Singleton {
	...
	public function set_assets() {
		...
		if ( Main::is_checkout() ) {
			...
			if ( 'yes' == $this->get_settings_manager()->get_setting( 'enable_address_autocomplete' ) && ! apply_filters( 'cfw_google_maps_compatibility_mode', false ) ) {
		...
		$cfwEventData = apply_filters(
			'cfw_event_data', array(
				...
				'settings'        => array(
					...
					'enable_checkout_login_reminder'          => 'yes' === get_option( 'woocommerce_enable_checkout_login_reminder' ) && apply_filters( 'cfw_suppress_default_login_form', $this->get_settings_manager()->get_setting( 'login_style' ) !== 'woocommerce' ) && Main::is_checkout(),
					...
					'enable_one_page_checkout'                => $this->get_settings_manager()->get_setting( 'enable_one_page_checkout' ) == 'yes',
					...
					'field_persistence_excludes'              => apply_filters( 'cfw_field_data_persistence_excludes', array(
						'input[type="file"]',
						'input[type="hidden"]',
						'input[type="submit"]',
						'input[type="reset"]',
	...
	public function add_plugin_hooks() {
		// Allow bypassing checkout template from 3rd party code
		if ( apply_filters( 'cfw_bypass_templates', false ) ) {
			return;
		}

		if ( $this->get_settings_manager()->get_setting('enable_address_autocomplete' ) == 'yes' ) {
		...
		if ( $this->is_enabled() ) {
			...
			if ( $this->get_settings_manager()->get_setting( 'enable_thank_you_page' ) == 'yes' && $this->get_settings_manager()->get_setting( 'override_view_order_template' ) == 'yes' ) {
			 ...
			 * User matching
			 */
			if ( $this->get_updater()->get_license_activation_limit() >= 5 && $this->get_settings_manager()->get_setting( 'user_matching' ) == 'enabled' ) {
	...
	function one_page_checkout_layout() {
		if ( $this->get_settings_manager()->get_setting( 'enable_one_page_checkout' ) === 'yes' ) {
			// Remove breadcrumbs
	...
	public static function activation() {
		...
		$main->get_templates_manager()->getActiveTemplate()->init();
	...
	function add_cart_image_size() {
		add_image_size( 'cfw_cart_thumb', apply_filters( 'cfw_cart_thumb_width', 60 ), apply_filters( 'cfw_cart_thumb_height', 0 ), apply_filters( 'cfw_crop_cart_thumbs', false ) );
	}
	...
	function get_theme_color_settings() {
		...
		if ( $this->get_templates_manager()->getActiveTemplate()->supports( 'header-background' ) ) {
		...
		if ( $this->get_templates_manager()->getActiveTemplate()->supports( 'footer-background' ) ) {
		...
		if ( $this->get_templates_manager()->getActiveTemplate()->supports( 'summary-background' ) ) {
			$color_settings['summary_background_color'] = cfw__( 'Summary Background Color', 'checkout-wc' );
		}

		if ( $this->get_templates_manager()->getActiveTemplate()->supports( 'accent-color' ) ) {
