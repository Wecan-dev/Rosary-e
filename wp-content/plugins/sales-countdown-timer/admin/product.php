...
class SALES_COUNTDOWN_TIMER_Admin_Product {
	...
	public function woocommerce_variation_options_pricing( $loop, $variation_data, $variation ) {
		...
		echo '<p class="form-field form-row form-row-first"><label for="_woo_ctr_progress_bar_goal' . $loop . '">' . esc_html__( 'Goal', 'sales-countdown-timer' ) . '</label>' . wc_help_tip( esc_html__( 'Your product goal', 'sales-countdown-timer' ) ) . '<input type="number" value="' . ( get_post_meta( $variation->ID, '_woo_ctr_progress_bar_goal', true ) ? ( get_post_meta( $variation->ID, '_woo_ctr_progress_bar_goal', true ) ) : '' ) . '" min="0" name="_woo_ctr_progress_bar_goal[' . $loop . ']" id="_woo_ctr_progress_bar_goal' . $loop . '"></p>';
		echo '<p class="form-field form-row form-row-last"><label for="_woo_ctr_progress_bar_initial' . $loop . '">' . esc_html__( 'Initial quantity', 'sales-countdown-timer' ) . '</label>' . wc_help_tip( esc_html__( 'This is the virtual quantity of sold products', 'sales-countdown-timer' ) ) . '<input type="number" value="' . ( get_post_meta( $variation->ID, '_woo_ctr_progress_bar_initial', true ) ? ( get_post_meta( $variation->ID, '_woo_ctr_progress_bar_initial', true ) ) : '' ) . '" min="0" name="_woo_ctr_progress_bar_initial[' . $loop . ']" id="_woo_ctr_progress_bar_initial' . $loop . '"></p>';
		echo '</div>';
	}
	...
	public function woocommerce_save_product_variation( $variation_id, $i ) {
		...
		update_post_meta( $variation_id, '_sale_price_dates_to', ( $date_on_sale_to - $gmt_offset * 3600 ) > 0 ? ( $date_on_sale_to - $gmt_offset * 3600 ) : '' );
