...
class Woo_Stock_Sync_Ui {
	...
	public function log() {
		...
		$results = Woo_Stock_Sync_Logger::get_all( $this->get_pagenum(), $per_page, $filter_by_product_id );
	...
	public function report() {
		...
		if ( isset( $_GET['wss_product_search'] ) && ! empty( trim( $_GET['wss_product_search'] ) ) ) {
			...
			$query->set( 's', trim( $_GET['wss_product_search'] ) );
			$query->set( 'order', 'ASC' );
	...
	private function urls() {
		return [
			...
			'create_log_table' => add_query_arg( [
				'action' => 'wss_create_log_table',
